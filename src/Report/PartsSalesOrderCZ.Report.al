report 80022 "DPG Parts Sales Order CZ" // 1057619 "Parts Sales Order CZ"
{
    // // @ Area PartsSales
    // +----------------------------------------------+
    // | Copyright (c) incadea                        |
    ...
    // PA032651   14.12.11 BL 5788134: Rounding of Amounts to calculate VAT Amount correctly.
    ...
    // PA033019   18.06.12 GB 6228969: Layout Changes for Vat
    ...
    //                        - Logos are not incorporated in RDLC layout
    // PA033336   28.09.12 8787455: Decimal controls will be dynamically formatted based on Regional settings.
    ...
    // PA036090   12.09.14 GI 28416 RDLC layout corrections
    ...
    // PA035484   02.04.14 GF 20895 Modified code for PrintFooterLineG to get initial value
    //                        from report selection setup.
    // PA036549   14.01.15 GF 32572 RDLC layout corrections - Unnecessary list Pagebreak removed,
    //                        Footer length reduction, Footer images adjustment.
    //                        Added missing code in OnOpenPage().
    // PA037341   22.01.15 GJ Merge N.6.1.4 to N.7.1.0 of PA035395.18593,PA035484.20895
    // PA038227   12.08.15 GN Changed Header information.
    //                        Added Footer functionality.
    // PA038227   24.08.15 GN 43660 Changed Footer fields sequence.
    // PA038218   18.09.15 IS 44878 "Sales Person" field left indented and size of text box is extended in header.
    // PA041125   21.07.16 GV 66952 RDLC Layout Modifications
    // PA041125   22.07.16 GQ 66952 RDLC Layout Modifications
    ...
    // PA041125   12.08.16 GV 68122 RDLC Layout corrections
    ...
    // PA041187   10.04.17 MS 93163 Remove option "Print Footer Line" from Request page and related logic. Provide missing logic for "Print Footer Line" option.
    // 
    ...
    // --- NAVCZ ---
    // API.1      01.03.18 PV New object created as a copy of Report 5028807 Parts Sales Order
    ...
    //                        SK legislation, Barcode
    // API.1.0.3  07.08.18 PV RDLC Layout changed - Lines for Notes removed
    // --- API Customization ---
    DefaultLayout = RDLC;
    RDLCLayout = './src/Report/PartsSalesOrderCZ.rdlc';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            RequestFilterHeading = 'Parts Sales Order';
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Header Text"; "Sales Document Text")
                    {
                        ...
                        column(Header_Text__Header_Text___Line_No__; "Header/Footer")
                        {
                            Description = 'PA033336';
                        }
                    ...
                    dataitem("Sales Line"; "Sales Line")
                    {
                        ...
                        column(Sales_Line__Sales_Line__Type; "Sales Line".Type)
                        {
                            Description = 'PA033336';
                        }
                        ...
                        column(Sales_Line__VAT_Identifier__Control1109400008; "VAT Identifier")
                        {
                            Description = 'PA033042,PA032651';
                        }
                        ...
                        column(Sales_Line__Unit_Price__Control1109400002; "Unit Price")
                        {
                            AutoFormatExpression = "Sales Header"."Currency Code";
                            AutoFormatType = 2;
                        }
                        column(Sales_Line_Quantity_Control1109400003; Quantity)
                        {
                        }
                        column(Sales_Line__Unit_of_Measure__Control1109400004; "Unit of Measure")
                        {
                        }
                        ...
                        column(Sales_Line__VAT_Identifier__Control1109400009; "VAT Identifier")
                        {
                            Description = 'PA033042,PA032651';
                        }
                        ...
                        column(CalcVATamount__Sales_Line___Amount_Including_VAT___Sales_Line__Amount__Exch__Parts_Tax_Amount__; CalcVATamount("Sales Line"."Amount Including VAT", "Sales Line".Amount, "Exch. Parts Tax Amount"))
                        {
                            AutoFormatExpression = "Sales Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        ...
                        column(VATpercent_SL; "Sales Line"."VAT %")
                        {
                        }
                        column(VATpercentCpn_SL; "Sales Line".FIELDCAPTION("VAT %"))
                        {
                        }
                        column(VINCpn_SL; "Sales Line".FIELDCAPTION(VIN))
                        {
                        }
                        column(VIN_SL; "Sales Line".VIN)
                        {
                        }
                        ...
                        begin
                            ...
                            VATAmountLineG."VAT Calculation Type" := "VAT Calculation Type";
                            VATAmountLineG."Tax Group Code" := "Tax Group Code";
                            VATAmountLineG."Sales Amount" := Amount;
                            // Stop PA032651
                            ...
                            // Stop PA032441
                            // Start PA033042
                            IF "Sales Line"."VAT Identifier" <> '' THEN
                    ...
                    dataitem(VATCounter; "Integer")
                    {
                        ...
                        column(VATAmountLine_COUNT_Control1101315996; VATAmountLineG.COUNT)
                        {
                            Description = 'PA033336';
                        }
                        ...
                        begin
                            ...
                            // Start PA032651
                            VATAmountLineG."Amount Including VAT" :=
                              ROUND(VATAmountLineG."Amount Including VAT", CurrencyG."Amount Rounding Precision");
                            VATAmountLineG."Exch. Parts Tax Amount" :=
                              ROUND(VATAmountLineG."Exch. Parts Tax Amount", CurrencyG."Amount Rounding Precision");
                            VATAmountLineG."Sales Amount" := ROUND(VATAmountLineG."Sales Amount", CurrencyG."Amount Rounding Precision");
                            VATAmountLineG."VAT Amount" :=
                              VATAmountLineG."Amount Including VAT" - VATAmountLineG."Exch. Parts Tax Amount" -
                              VATAmountLineG."Sales Amount";
                            // Stop PA032651
                        end;
                    ...
                    dataitem("Footer Text"; "Sales Document Text")
                    {
                        ...
                        column(Footer_Text__Footer_Text___Line_No__; "Header/Footer")
                        {
                            Description = 'PA033336';
                        }
                    ...
                    dataitem(SignatureTexts; "Integer")
                    {
                        DataItemTableView = SORTING(Number) WHERE(Number = CONST(1));
                    ...
                    dataitem(VATText; "Integer")
                    {
                        ...
                        dataitem(VATTextLine; "VAT Text Line")
                        {
                            ...
                            begin
                                ...
                                VATLegendsG := '';
                                IF IncrG = 1 THEN
                                    VATLegendsG := VATIdentifierTempG.Code + C_INC035;
                                IncrG := IncrG + 1;
                                // Stop PA035597
                            end;
                    ...
                    begin
                        ...
                        IF PrintBarCodeG THEN BEGIN
                            //Start API.1.0.2
                            BarCodeTextG := 'ö' + "Sales Header"."No." + 'ö';
                            //BarCodeTextG := '*' + "Sales Header"."No." + '*';
                            //Stop API.1.0.2
                        END;
                        // Stop PA035597
                    end;
            ...
            begin
                ...
                IsRepSelectionSetupAvailableG := ReportSelectionG.GetFooterDetails(ReportLayoutSetupG, "Shortcut Dimension 2 Code", "Location Code",
                  REPORT::"DPG Parts Sales Order CZ"); //Start API.1      01.03.18 PV
                ...
                IF NOT CurrReport.PREVIEW THEN BEGIN
                    LogInteractionG := SegMgntG.FindInteractTmplCode(3) <> '';
                    IF LogInteractionG THEN
                        SegMgntG.LogDocument(
                          3, "No.", 0, 0, DATABASE::Customer, "Bill-to Customer No.",
    ...
    var
        ...
        C_INC034: Label 'Parts Sales-Order Confirm.';
        C_INC035: Label ' =';
    ...
    [Scope('Internal')]
    procedure CalcVATamount(AmountInclVATP: Decimal; AmountP: Decimal; ExchangePartsTaxAmountP: Decimal): Decimal
    begin
        // @ The function should calculate a rounded correct value of VAT Amount, based on the used parameter.
        // @ param AmountInclVATP : Amount inclusive VAT amount.
        // @ param AmountP : Amount without VAT.
        // @ param ExchangePartsTaxAmountP : Tax amount for Exchange Parts.
        // @ retval : Correct rounded VAT amount.
        // Start PA032651
        AmountInclVATP := ROUND(AmountInclVATP, CurrencyG."Amount Rounding Precision");
        AmountP := ROUND(AmountP, CurrencyG."Amount Rounding Precision");
        ExchangePartsTaxAmountP := ROUND(ExchangePartsTaxAmountP, CurrencyG."Amount Rounding Precision");
        EXIT(AmountInclVATP - AmountP - ExchangePartsTaxAmountP);
        // Stop PA032651
    end;

    local procedure GetCurrency()
    begin
        // @ The procedure has to read the corresponding Currency record based on the currency code of Sales Header.
        // Start PA032651
    ...
    begin
        ...
        // Start PA041187.93163
        // Stop PA041187.93163
        // API.1      01.03.18 PV note: PrintAddrBlockG is usually used to (not)show just for some data ("VAT Registration No.") in layout
        //    and (not) fill email, fax and phone - usage is not consistent and finaly used on layout without effect
